...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($mail_err) && empty($password_err) && empty($confirm_password_err) && empty($name_err) && empty($fecha_err) && empty($nacio_err)){
        ...
        $sql2 = "INSERT INTO usuarios (uid, nombre, nacimiento, correo, nacionalidad, password) VALUES (:id, :nombre, :fecha, :mail, :nacionalidad, :password)";
        if($stmt = $db->prepare($sql)){
            ...
            $stmt2->bindParam(":mail", $param_mail, PDO::PARAM_STR);
            $stmt2->bindParam(":password", $param_password, PDO::PARAM_STR);
            $stmt2->bindParam(":id", $param_id, PDO::PARAM_STR);
            $stmt2->bindParam(":nombre", $param_nombre, PDO::PARAM_STR);
            $stmt2->bindParam(":fecha", $param_fecha, PDO::PARAM_STR);
            $stmt2->bindParam(":nacionalidad", $param_nacio, PDO::PARAM_STR);
            ...
            if($stmt->execute()){
                ...
                $arr = array("idu" => $param_id, "nombre" => $param_nombre, 
                             "fecha_nac" => $param_fecha, "correo" => $param_mail,
                             "nacionalidad" => $param_nacio, "password" => $param_password 
                            );
                $data = json_encode($arr, JSON_NUMERIC_CHECK);
                $datos = CallAPI("POST", "https://young-waters-69541.herokuapp.com/register_user", $data);
